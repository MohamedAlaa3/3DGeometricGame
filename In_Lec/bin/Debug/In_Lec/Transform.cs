...
internal class Transform
{
	...
	public static void Rotatex(_3D_Model a, double theta)
	{
		for (int i = 0; i < a.L_3D_Pts.Count; i++)
		{
			double y1 = (double)a.L_3D_Pts[i].Y * Math.Cos(theta);
			double z1 = (double)a.L_3D_Pts[i].Z * Math.Sin(theta);
			double z2 = (double)a.L_3D_Pts[i].Z * Math.Cos(theta);
			double y2 = (double)a.L_3D_Pts[i].Y * Math.Sin(theta);
	...
	public static void Rotatey(_3D_Model a, double theta)
	{
		for (int i = 0; i < a.L_3D_Pts.Count; i++)
		{
			double z1 = (double)a.L_3D_Pts[i].Z * Math.Cos(theta);
			double x1 = (double)a.L_3D_Pts[i].X * Math.Sin(theta);
			double x2 = (double)a.L_3D_Pts[i].X * Math.Cos(theta);
			double z2 = (double)a.L_3D_Pts[i].Z * Math.Sin(theta);
	...
	public static void Rotatez(_3D_Model a, double theta)
	{
		for (int i = 0; i < a.L_3D_Pts.Count; i++)
		{
			double x1 = (double)a.L_3D_Pts[i].X * Math.Cos(theta);
			double y1 = (double)a.L_3D_Pts[i].Y * Math.Sin(theta);
			double y2 = (double)a.L_3D_Pts[i].Y * Math.Cos(theta);
			double x2 = (double)a.L_3D_Pts[i].X * Math.Sin(theta);
	...
	public static void Rotateall(_3D_Model a, _3D_Point p1, _3D_Point p2, int sign)
	{
		...
		double oldy = p1.Y;
		double oldz = p1.Z;
		Translate(a, 0f - p1.X, 0f - p1.Y, 0f - p1.Z);
		double v1 = p1.X - p2.X;
		double v2 = p1.Y - p2.Y;
		double v3 = p1.Z - p2.Z;
